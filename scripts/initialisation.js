...
const database_statement = "CREATE TABLE accounts (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username TEXT, password TEXT, role TEXT, stories JSON1)"
                                    ...
                                    try {
                                        cryptoController.hashPassword(username, password, function callback(err, result) {
                                            if (err) {
                                                ...
                                                dbController.storeUser(username, result, "admin", function callback(err, result) {
                                                    if (err) {
                                                        throw err;
                                                    }
                                                    else {
